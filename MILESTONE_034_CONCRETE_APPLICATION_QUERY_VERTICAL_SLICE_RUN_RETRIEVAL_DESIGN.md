...
**Correction history:** An independent hostile design review found two findings. `M034-DESIGN-REVIEW-0001` (MAJOR, BLOCKING): Section 9 originally described `Run.version` — the aggregate's own domain-state field, which advances through every lifecycle transition and through `append_manifest()`, and is embedded in every `StateTransitionRecord` — using language that actually belongs to `LoadedAggregate.persisted_version` (the separate, repository-loaded concurrency token consumed by `RunRepository.save(..., expected_persisted_version=...)`), materially conflating the two. `M034-DESIGN-REVIEW-0002` (MINOR): Section 19 overstated that "field typing is enforced ... by the dataclass mechanism itself" — a plain Python dataclass does not runtime-enforce annotated field types; only `DomainIdentity`/`RunId`'s own `__post_init__` construction-time checks do. Both are corrected throughout this document (Sections 8-11, 19, 25-27, 34). The selected capability, query identity model, handler shape, single-`get()` sequence, transparent error behavior, architecture boundary, and PostgreSQL strategy are unchanged.
...
| B | Return `LoadedAggregate[Run]` directly | Same mutability leakage as A (the `.aggregate` field is the same mutable `Run`), plus it additionally exposes `LoadedAggregate.persisted_version` — the repository-loaded optimistic-concurrency token that `RunRepository.save(aggregate, *, expected_persisted_version=...)` consumes, identically in kind to `CampaignRepository.save()`. This is distinct from `Run.version`, the aggregate's own domain-state field (Section 9); `persisted_version` specifically is repository/persistence metadata a read-only query caller has no legitimate use for, and exposing it invites an out-of-band `save()` call that bypasses the intended command-handler write path this project has consistently kept separate (M030-M033). Rejected. |
...
- **`Run.version` (`AggregateVersion`, the aggregate's own domain-state field) — Aggregate-Version Decision.** Repository truth, verified directly in `run/aggregate.py`: `Run.version` is genuine aggregate domain state, not persistence metadata. It advances on every lifecycle transition (`Run._transition()`: `next_version = self._version.next()`, then `self._version = next_version`) and on every `Run.append_manifest()` call (same pattern), and its value is embedded directly in each `StateTransitionRecord.version` field. It is a wholly separate field from `LoadedAggregate.persisted_version` (Section 8, Candidate B; Section 10), which is repository-loaded metadata consumed by `save()`. Two options were evaluated on Run-specific merits (not M031 symmetry): **(A) include `version: AggregateVersion` on `RunSnapshot`** — advantages: exposes meaningful aggregate state, lets a caller observe logical Run evolution, avoids a "snapshot" that omits versioned domain state; disadvantages: enlarges the result surface, and — critically — creates a real risk that a future caller mistakes `RunSnapshot.version` for `RunRepository.save()`'s `expected_persisted_version` argument, since the two are numerically equal at load time (`_reconstruct_run` sets `run._version = state.version`, the same durable `version` column `persisted_version` is read from) but diverge the moment either the in-memory aggregate mutates further or a concurrent writer commits — a genuine confusability hazard given `AggregateVersion`'s own docstring is "Optimistic-concurrency version for one aggregate root." **(B) exclude it, treating `RunSnapshot` explicitly as a narrow Run header/status result, not a complete aggregate snapshot** — advantages: preserves a deliberately bounded result, avoids the concurrency-token-confusion hazard identified above, and matches the Mission Statement's sole proof obligation (`QueryHandler`/`QueryEntryPoint` generalization), which the three included fields already fully discharge; disadvantages: a caller cannot observe how many times a Run has mutated from this result alone. **Selected: Option B.** The confusion hazard is concrete and specific to Run's own semantics (both fields numerically coincide at load time, unlike anything comparable for Campaign), not a restatement of M031 precedent — though it happens to be consistent with it: `CampaignSnapshot` likewise excludes `Campaign.version` (verified: `CampaignSnapshot.__slots__ == {"identity", "scope_statement", "state"}`), confirming this codebase's own established meaning of "Snapshot" is already "a bounded read-only view of current descriptive state," not "every aggregate field including its version counter" — so the type name `RunSnapshot` remains truthful under Option B and is not renamed.
...
`manifests` and `transition_history` are Run-owned state, not cross-aggregate data — their exclusion here is a deliberate scope/design choice about how much of one aggregate's own state a minimal first read-side proof needs to expose, not a claim that they are irrelevant. Because `RunSnapshot` deliberately excludes them (and `Run.version`), it is a bounded Run header/status view, not a claim of complete Run state — a distinction made explicit here precisely because "Snapshot" could otherwise be misread as implying completeness (Section 1 correction history, `M034-DESIGN-REVIEW-0001`).

`RunSnapshot` is a `@dataclass(frozen=True, slots=True)` — immutable, no default values, exactly three fields (`identity`, `campaign_id`, `state`), mirroring `CampaignSnapshot`'s dataclass posture (verified in `usecases/get_campaign.py`) because that posture (frozen, slots, no extra machinery) is independently the correct choice for any minimal read-value in this codebase, not because of blind mirroring.

---

## 10. Persisted-Version Decision

`LoadedAggregate.persisted_version` — the repository-loaded optimistic-concurrency token, distinct from `Run.version` (Section 9) — is excluded from `RunSnapshot`. `GetRunHandler` reads it off the `LoadedAggregate[Run]` returned by `RunRepository.get()` (it is present on the object) but never places it on the constructed `RunSnapshot`, exactly as `GetCampaignHandler` reads but never exposes it for Campaign (verified: `CampaignSnapshot.__slots__` contains no `persisted_version`, confirmed by the existing test `test_snapshot_contains_no_persisted_version`).

This decision is made explicit, not left implicit: **M034 is a read-only retrieval query and does not support obtaining the `expected_persisted_version` an M032-style update would need; that capability remains separately deferred (Option A of the two considered).** The alternative — including `persisted_version` on `RunSnapshot` because read-then-update is a legitimate application workflow (Option B) — is rejected on direct evidence from the already-frozen M032 pattern: `PrepareCampaignForAuthorizationCommand` (verified in `usecases/prepare_campaign_for_authorization.py`) already requires `expected_persisted_version` as a field the *caller* supplies directly on the command, not a value sourced from a prior query result. That is the existing, frozen mechanism by which this project's write path obtains an expected version, and it is orthogonal to any query. Introducing `persisted_version` into M034's result would speculatively support a read→update orchestration workflow this project's own frozen precedent does not structurally need, and would reintroduce exactly the persistence-metadata-leakage risk Section 8 (Candidate B) already rejected. If a genuine read→update usability gap is ever evidenced, it is a separate, independently-scoped future capability — not implicitly granted by this query's result shape.

---

## 11. Run State/Manifest/History Representation

`manifests` and `transition_history` are Run-owned state — not cross-aggregate data, not irrelevant data — that `PostgresRunRepository.get()` eagerly reconstructs as an unavoidable side effect of full aggregate reconstruction (Section 5), because `RunRepository.get()` is a frozen M020/M023 contract this milestone must not modify and it has no partial-load variant. `GetRunHandler` never reads either collection, and neither appears on `RunSnapshot`. This is a deliberate M034 scope/design choice — this milestone intentionally returns a bounded result (Section 9) rather than the complete set of Run-owned collections — not a judgment that manifests or transition history are unimportant; their future exposure remains explicitly deferred (Section 30), and this design does not describe `RunSnapshot` as a complete Run snapshot (Section 9). The eager loading is a real, acknowledged I/O cost (the repository does strictly more work than the query result needs) accepted because the frozen contract offers no alternative. PostgreSQL evidence (Section 24) must still confirm this loading occurs correctly (regression coverage), even though its result is discarded.
...
- **Query construction:** `GetRunQuery` is a passive, immutable typed carrier. A plain Python dataclass — `frozen=True`/`slots=True` included — does not runtime-enforce its annotated field types; `GetRunQuery(identity=identity)` would not raise if `identity` were, hypothetically, some other object entirely. `frozen=True` enforces immutability after construction (blocks reassignment), not the field's type. Callers are expected to supply a valid `DomainIdentity[RunId]`; static type checking (mypy) documents and verifies this intended usage at development time, matching M031's own posture for `GetCampaignQuery`.
- **`RunId`/`DomainIdentity`:** the actual runtime validation guarantee comes entirely from these types' own frozen `__post_init__` checks (`Identifier.__post_init__` regex match, `DomainIdentity.__post_init__` isinstance checks), not from `GetRunQuery`'s dataclass mechanism. Because a caller must already possess a constructed `DomainIdentity[RunId]` to populate the field, and that construction path already enforces validity, a `GetRunQuery` carrying a genuinely invalid identity is not reachable through normal value-object construction — but this is a property of `DomainIdentity`/`RunId`, not of `GetRunQuery` itself.
...
- **Handler:** performs no independent or duplicate validation of its own — it trusts the query's `identity` field exactly as received, exactly as `GetCampaignHandler` does for `GetCampaignQuery`.
...
**B. Handler success** — sole dependency is `RunRepository` (a recording fake, mirroring `_RecordingCampaignRepository`); `get()` called exactly once; exact identity object passed through unchanged (`is` comparison); no `CampaignRepository` dependency exists to call; no second repository operation; `RunSnapshot` fields match the loaded aggregate's `identity`/`campaign_id`/`state` exactly; `set(RunSnapshot.__slots__) == {"identity", "campaign_id", "state"}`, explicitly proving both `LoadedAggregate.persisted_version` and `Run.version` (the aggregate's own field, deliberately excluded per Section 9) are absent — the test/assertion language must name each exclusion accurately and never call `Run.version` "persistence metadata"; the source `Run` aggregate is not mutated by retrieval (its `campaign_id`, `state`, and `version` are unchanged after `handle()` returns, mirroring `test_campaign_aggregate_is_not_mutated`); synchronous behavior.
...
| Result contract | A (raw `Run`), B (`LoadedAggregate[Run]`), D (existing frozen type) | C (new `RunSnapshot`) | A/B leak mutability (and B additionally leaks `LoadedAggregate.persisted_version`); D — no existing type carries the needed shape. |
| `persisted_version` exposure | Include on `RunSnapshot` | Excluded (Section 10, Option A) | Repository-loaded concurrency-token metadata with no legitimate read-only use; M032's own frozen pattern already sources `expected_persisted_version` as a caller-supplied command field, not from a query; risk of inviting out-of-band writes. |
| `Run.version` (aggregate-state field) exposure | Include `version: AggregateVersion` on `RunSnapshot` (Option A, Section 9) | Excluded (Option B, Section 9) | Not persistence metadata — genuine domain state — but excluded because it numerically coincides with `persisted_version` at load time, creating a concrete confusability hazard with the deliberately-excluded concurrency token, and because the three included fields already fully discharge the Mission Statement's proof obligation. |
| `next_transition_sequence`/`transition_history`/`manifests` on `RunSnapshot` | Include | Excluded, all three | Unbounded size growth, audit-metadata exposure (for `transition_history`), and (for `manifests`) an independent unresolved representation question outside this milestone's authorized scope. |
...
Restated for clarity from Section 26 with full reasoning: raw `Run` (Section 8-A); `LoadedAggregate[Run]` (Section 8-B); any existing frozen type as the result (Section 8-D); governance-ID-only query identity (Section 6-D); split-field query identity (Section 6-B); raw-string query identity (Section 6-C); `LoadedAggregate.persisted_version` on the snapshot (Section 10); `Run.version`, the aggregate's own domain-state field, on the snapshot (Section 9, Aggregate-Version Decision, Option A rejected in favor of Option B); `manifests`/`transition_history`/`next_transition_sequence` on the snapshot (Section 9); any error-translation or nullable-result not-found behavior (Section 17); any application-level transaction orchestration (Section 20); any composition-root, registry, dispatcher, mediator, or service-locator binding (Section 21); any new architecture-checker permission (Section 23).
...
- **Persistence metadata leakage:** mitigated by excluding `LoadedAggregate.persisted_version` (Section 10).
- **Aggregate-version/persisted-version confusion:** mitigated by excluding both `Run.version` and `persisted_version` from `RunSnapshot` (Section 9 Aggregate-Version Decision, Section 10), and by naming and defining each precisely everywhere in this document — the two are never described as the same field or the same kind of metadata.
...
- **Omission of important Run state:** the exclusion of `Run.version`/`manifests`/`transition_history` is a deliberate, independently justified narrowing (Section 9), not an oversight; it is recorded explicitly as deferred work (Section 30), not silently dropped.
...
**Round 1 (design-candidate self-review), attacked directly against the Phase 20 attack list:**
...
- **Dropped persisted version without justification:** justified explicitly (Section 10).
...
This round did not catch the conceptual conflation an independent hostile design review subsequently found (Section 1 correction history) — recorded honestly rather than silently superseded.

**Round 2 (post-correction hostile self-audit), against the independent review's own findings and Phase 12's attack list:**

- **Version described as persisted metadata:** absent — Section 9's Aggregate-Version Decision now describes `Run.version` correctly as aggregate domain state (advancing via `_transition()` and `append_manifest()`, embedded in `StateTransitionRecord`), never as persistence or write-side-only metadata.
- **Aggregate version and persisted version conflated:** absent — every remaining reference distinguishes `Run.version` from `LoadedAggregate.persisted_version` by name and by definition (Sections 8, 9, 10, 19, 26, 27, 31); grepped for `` `version` `` in isolation across the corrected document to confirm no remaining ambiguous reference.
- **"Consumed only by save":** now stated only of `persisted_version`/`expected_persisted_version` (Sections 8-B, 10), never of `Run.version`.
- **Misleading snapshot-completeness claims:** Section 9 and Section 11 now state explicitly that `RunSnapshot` is a bounded Run header/status view, not a complete snapshot of Run's own state, and explain why the type name remains truthful regardless.
- **Dataclass runtime type-enforcement claims:** corrected in Section 19 — a plain dataclass does not runtime-enforce annotated field types; the actual guarantee comes from `DomainIdentity`/`RunId`'s own `__post_init__` construction.
- **Unresolved version inclusion:** resolved — Option B (exclude) explicitly selected with Run-specific reasoning (Section 9), not left open.
- **Persisted-version ambiguity:** resolved — Option A explicitly selected (M034 is read-only; read→update version-sourcing is deferred), with direct M032 evidence (Section 10).
- **Test-strategy mismatch:** Section 25.B updated to assert the exact `RunSnapshot.__slots__` set and to require exclusion language that never calls `Run.version` persistence metadata.
- **PostgreSQL-strategy mismatch:** Section 24 required no change — it never referenced `version` at the field level; unaffected by the correction.
- **Second `get()`, Campaign lookup, generic-read-model creep, M035 leakage:** all re-confirmed absent, unchanged from Round 1.
- **No `__post_init__`, runtime `isinstance` check, or duplicated validation was added anywhere in this correction**, per the correction mission's own constraint.
...
**CANDIDATE_FOR_FINAL_INDEPENDENT_DESIGN_RE_REVIEW.** Not approved. Not frozen. Does not authorize implementation.

**Next permitted action:** MILESTONE-034 FINAL INDEPENDENT DESIGN RE-REVIEW.
