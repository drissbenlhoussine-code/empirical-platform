...
- **`RunId`/`DomainIdentity`/`AggregateVersion`:** `RunId.__post_init__` validates the governance-identifier format (frozen regex check); `AggregateVersion.__post_init__` validates non-negativity; both reject invalid values at construction, so an `AuthorizeRunCommand` cannot be constructed with a malformed `RunId` or a negative version in the first place. `DomainIdentity.__post_init__` validates only the base identity-pair *structure* at runtime — `isinstance(governance_id, Identifier)` and `isinstance(runtime_id, RuntimeIdentifier)` — it does not and cannot runtime-enforce the specific generic specialization `DomainIdentity[RunId]`, since Python erases generic type parameters at runtime; nothing prevents constructing a `DomainIdentity` whose `governance_id` is some other `Identifier` subtype (e.g. a `CampaignId`) and treating it, via static annotation only, as `DomainIdentity[RunId]`. `RunId` specialization is therefore expressed through static typing (mypy) and enforced downstream only by whatever the `Run`/`RunRepository` contracts actually do with the value (e.g. SQL parameter binding, reconstruction) — not by `DomainIdentity` itself. The handler performs no additional runtime type check to compensate for this, matching every prior milestone's established validation-ownership discipline (no duplicated or invented enforcement beyond what the frozen types already provide).
