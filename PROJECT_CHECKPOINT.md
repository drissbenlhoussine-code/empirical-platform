...
M034_DESIGN_STATUS=CANDIDATE_FOR_INDEPENDENT_DESIGN_REVIEW
...
M034_STATUS=DESIGN_CANDIDATE
M035_STATUS=NOT_STARTED
NEXT_PERMITTED_ACTION=MILESTONE-034 INDEPENDENT DESIGN REVIEW
```
...
**Design document:** `MILESTONE_034_CONCRETE_APPLICATION_QUERY_VERTICAL_SLICE_RUN_RETRIEVAL_DESIGN.md` (design candidate commit `d343e38cba9b5a49db278c72ca1650dd50839bd2`).
...
**Result contract (the central load-bearing decision):** a new milestone-local `RunSnapshot` (`identity`, `campaign_id`, `state` — three fields, frozen/slots, no `persisted_version`) — selected over returning the raw `Run` aggregate (mutability leakage: `Run` exposes seven lifecycle-transition methods plus `append_manifest`), `LoadedAggregate[Run]` (same leakage plus `persisted_version` exposure), or any existing frozen type (none carries the needed shape). `version`, `next_transition_sequence`, and `transition_history` are excluded as unbounded write-side/audit metadata; `manifests` is independently excluded (not by analogy to `CampaignSnapshot`, which has no equivalent field) as an unbounded, cross-package collection whose read-side representation is an unresolved question outside this milestone's authorized scope — deferred to a future, independently-scoped milestone.
...
**Hostile self-review:** performed by the design mission itself against a fifteen-item attack list (governance-ID-only lookup, identity reconstruction ambiguity, extra `get()` calls, Campaign lookup, raw-aggregate/`LoadedAggregate` leakage, incomplete snapshot fields, unjustified persisted-version omission, manifests/history aliasing, not-found translation ambiguity, result-wrapper creep, listing/filtering/projection leakage, generic read-model abstraction, checker mismatch, production composition leakage, M035 leakage) — no issue survived requiring correction.
