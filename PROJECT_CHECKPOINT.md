...
M034_SCOPE_STATUS=CANDIDATE_FOR_INDEPENDENT_SCOPE_REVIEW
M034_SCOPE_COMMIT=PENDING
M034_STATUS=SCOPE_CANDIDATE
M035_STATUS=NOT_STARTED
NEXT_PERMITTED_ACTION=MILESTONE-034 INDEPENDENT SCOPE REVIEW
```
...
**Scope document:** `MILESTONE_034_CONCRETE_APPLICATION_QUERY_VERTICAL_SLICE_RUN_RETRIEVAL_SCOPE.md` (scope candidate commit `PENDING`, to be recorded in a narrow follow-up commit).
...
**Open design questions (explicitly not resolved by scope):** the exact query/handler type names and shapes; the exact Run read-value type's fields (expected candidates: `identity`, `campaign_id`, `state`, mirroring `CampaignSnapshot`'s own shape, but not frozen here); whether write-side metadata (e.g. `persisted_version`) is deliberately excluded, mirroring M031's own justified exclusion.

**Status:** `CANDIDATE_FOR_INDEPENDENT_SCOPE_REVIEW`. Not approved. Not frozen. Does not authorize design or implementation.

**Next permitted action:** MILESTONE-034 INDEPENDENT SCOPE REVIEW.
