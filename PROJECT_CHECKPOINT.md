...
The design adds exactly one new primitive, `PostgresComposedUnitOfWork` (plus a private `_JoinedUnitOfWork` wrapper and one new `ContextVar`), entirely inside the existing `src/empirical_platform/shared/persistence/postgres.py` module. It changes **no** M020 Repository Protocol signature and **no** M023 concrete repository adapter source file — all four adapters participate correctly with zero changes, because they already only ever call `self._service.unit_of_work()` without inspecting its concrete type. Single-operation behavior with no composed scope open is unchanged byte-for-byte from today's frozen M023 behavior.

A hostile self-review pass caught and corrected one genuine mechanism defect before this checkpoint was written: the composed scope's own `__enter__` must construct its underlying `PostgresUnitOfWork` directly rather than through the public, now-join-aware factory method, or a second composed scope opened while one is already active would incorrectly join instead of raising. See the Design document Section 17 (`M024-DESIGN-ISSUE-0005`) for the full account.

Full detail: `MILESTONE_024_MULTI_AGGREGATE_UNIT_OF_WORK_SCOPE_SELECTION.md`, `MILESTONE_024_MULTI_AGGREGATE_UNIT_OF_WORK_DESIGN.md`.

**This design is NOT approved, NOT frozen, and no repository/mapper/schema/application code has been created or modified by it.** It is ready for independent review, not yet reviewed. Repository runtime composition (Candidate E), application services (Candidate F), and retry policy (Candidate J) all remain independently deferred, not designed by this milestone.
