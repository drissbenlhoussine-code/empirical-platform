...
M032_STATUS=SCOPE_CANDIDATE
M033_STATUS=NOT_STARTED
NEXT_PERMITTED_ACTION=MILESTONE-032 INDEPENDENT SCOPE REVIEW
```
...
- MILESTONE-032 independent scope review and owner scope freeze (a scope candidate now exists — see Section 19 — not yet reviewed or frozen);
- retry-on-`OptimisticConcurrencyConflict` policy (explicitly named as the reason MILESTONE-032 exists; not itself in MILESTONE-032's scope — see the M032 scope document Section 22);
...
**Verified gap (evidence-driven, not assumed):** an 18-point architectural inventory across domain aggregates, repository Protocols, PostgreSQL adapters, transaction/runtime composition, CQRS contracts, existing use cases, transport, error/concurrency behavior, and every `PROJECT_CHECKPOINT.md` deferred-capability entry found that `CampaignRepository.save()` and the `OptimisticConcurrencyConflict` contract it guards — frozen since M020/M023 — have zero application-layer proof: `CreateCampaignHandler` (M030) calls only `add()`, `GetCampaignHandler` (M031) calls only `get()`. Both M030's own frozen scope ("blocked on a `save()`-based command that does not yet exist") and M031's own deferred-capabilities entry name this exact gap as the next dependency.
...
**Status:** `CANDIDATE_FOR_INDEPENDENT_SCOPE_REVIEW`. Not reviewed. Not approved. Not frozen. Does not authorize design or implementation.

**Next permitted action:** MILESTONE-032 INDEPENDENT SCOPE REVIEW.
