...
| `tests/unit/test_m085_identity_lineage.py` | ImportError | **Not a reproduction of anything.** The suite imports the corrected API (`order_terms_mismatches`, `attempt_may_have_transmitted`, `RECOGNIZED_DEFINITIVE_REFUSAL_CODES`) that `2726f6f` does not have, so it cannot run there. The behaviours B and B2 on `2726f6f` are evidenced differently: by the tests that *asserted* them and passed on `2726f6f` — `test_an_exact_match_found_before_sending_is_adopted_without_a_send`, `test_a_rebuilt_database_meets_the_brokers_existing_order_without_sending_again`, `test_a_duplicate_answer_after_the_send_is_reconciled_not_rejected` (adoption without lineage), `test_the_lookup_failing_before_the_send_means_nothing_is_sent` (terminal REJECTED on an inconclusive lookup) — all removed as superseded by `80d1faa` and listed in `tests/unit/test_m085_identity_collision.py`'s docstring. |
