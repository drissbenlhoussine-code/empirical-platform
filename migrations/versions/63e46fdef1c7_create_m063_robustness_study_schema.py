"""create m063 robustness study schema

Revision ID: 63e46fdef1c7
Revises: a66cb39e7dba
Create Date: 2026-08-10 00:00:00.000000

MILESTONE-063: creates `robustness_study` (one row per broad multi-window
robustness study) and `robustness_window` (N child rows, one per declared
window), immutable and single-insert like every non-lifecycle record since
M057 -- no `*_transition` child table. Each `robustness_window` row
references its own, independently persisted M061 `historical_backtest_run`
row by governance_id/runtime_id (the unmodified M061 table, unchanged by
this migration) -- full per-trade detail for every window remains
reachable through the existing, frozen M061 repository/CLI, never
duplicated here. Regime breakdown is a derived view over `robustness_window`
rows (`GROUP BY regime_label`), never separately stored.
"""
...
revision: str = "63e46fdef1c7"
down_revision: str | None = "a66cb39e7dba"
...
robustness_study = sa.Table(
    ...
    sa.CheckConstraint(
        "classification IN ('INSUFFICIENT_SAMPLE', 'ROBUSTNESS_EVIDENCE_RECORDED', "
        "'ROBUSTNESS_EVIDENCE_MIXED', 'ROBUSTNESS_EVIDENCE_WEAK')",
        name="classification_valid",
    ),
    sa.CheckConstraint("window_count >= 1", name="window_count_positive"),
)

robustness_window = sa.Table(
    "robustness_window",
    _METADATA,
    ...
    sa.ForeignKeyConstraint(
        ["study_runtime_id"], ["robustness_study.runtime_id"], ondelete="CASCADE"
    ),
    sa.ForeignKeyConstraint(
...
def upgrade() -> None:
    """Create the robustness_study and robustness_window tables."""
    bind = op.get_bind()
    robustness_study.create(bind=bind)
    robustness_window.create(bind=bind)


def downgrade() -> None:
    """Drop the robustness_window and robustness_study tables."""
    bind = op.get_bind()
    robustness_window.drop(bind=bind)
    robustness_study.drop(bind=bind)
