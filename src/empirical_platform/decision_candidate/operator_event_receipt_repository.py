...
class OperatorEventReceiptRepository(Protocol):
    ...
    def attest(
        ...
        the one that appended the event, MUST read the event back before taking
        the instant, and MUST take the instant only after that read. Assigning
        it inside the ingesting transaction was proved to leak knowledge.
