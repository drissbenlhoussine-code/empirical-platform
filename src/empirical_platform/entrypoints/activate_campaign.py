"""Real end-to-end Campaign activation composition root.
...
def run_activate_campaign(
    *,
    ...
    """Activate one Campaign, end-to-end, against real PostgreSQL."""
    with postgres_repository_runtime(config) as runtime:
        handler = ActivateCampaignHandler(campaign_repository=runtime.campaigns)
...
def main() -> None:
    """Activate one Campaign from CLI arguments, and print the result."""
    if len(sys.argv) not in (7, 8):
        raise SystemExit(
            "usage: empirical-platform-activate-campaign "
            "<governance_id> <runtime_id> <expected_version> <reason> <actor> <occurred_at_iso> "
            "[correlation_id]"
        )
    result = run_activate_campaign(
