"""Real end-to-end Run authorization composition root.

Composes the frozen MILESTONE-035 vertical slice through the shared
...
def run_authorize_run(
    *,
    ...
    """Authorize one Run, end-to-end, against real PostgreSQL."""
    with postgres_repository_runtime(config) as runtime:
        handler = AuthorizeRunHandler(run_repository=runtime.runs)
...
def main() -> None:
    """Authorize one Run from CLI arguments, and print the result."""
    if len(sys.argv) not in (6, 7, 8):
        raise SystemExit(
            "usage: empirical-platform-authorize-run "
            "<governance_id> <runtime_id> <expected_version> <actor> <occurred_at_iso> "
            "[correlation_id] [reason]"
        )
    result = run_authorize_run(
