"""Real end-to-end Review cancellation composition root.

Composes the frozen MILESTONE-049 vertical slice through the shared
...
def run_cancel_review(
    *,
    ...
    """Cancel one Review, end-to-end, against real PostgreSQL."""
    with postgres_repository_runtime(config) as runtime:
        handler = CancelReviewHandler(review_repository=runtime.reviews)
...
def main() -> None:
    """Cancel one Review from CLI arguments, and print the result."""
    if len(sys.argv) not in (7, 8):
        raise SystemExit(
            "usage: empirical-platform-cancel-review "
            "<governance_id> <runtime_id> <expected_version> <reason> <actor> <occurred_at_iso> "
            "[correlation_id]"
        )
    result = run_cancel_review(
