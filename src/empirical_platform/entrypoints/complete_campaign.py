"""Real end-to-end Campaign completion composition root.
...
def run_complete_campaign(
    *,
    ...
    """Complete one Campaign, end-to-end, against real PostgreSQL."""
    with postgres_repository_runtime(config) as runtime:
        handler = CompleteCampaignHandler(campaign_repository=runtime.campaigns)
...
def main() -> None:
    """Complete one Campaign from CLI arguments, and print the result."""
    if len(sys.argv) not in (7, 8):
        raise SystemExit(
            "usage: empirical-platform-complete-campaign "
            "<governance_id> <runtime_id> <expected_version> <reason> <actor> <occurred_at_iso> "
            "[correlation_id]"
        )
    result = run_complete_campaign(
