"""Real end-to-end Run execution-completion composition root.
...
def run_complete_run_execution(
    *,
    ...
    """Complete execution on one Run, end-to-end, against real PostgreSQL."""
    with postgres_repository_runtime(config) as runtime:
        handler = CompleteRunExecutionHandler(run_repository=runtime.runs)
...
def main() -> None:
    """Complete execution on one Run from CLI arguments, and print the result."""
    if len(sys.argv) not in (6, 7, 8):
        raise SystemExit(
            "usage: empirical-platform-complete-run-execution "
            "<governance_id> <runtime_id> <expected_version> <actor> <occurred_at_iso> "
            "[correlation_id] [reason]"
        )
    result = run_complete_run_execution(
