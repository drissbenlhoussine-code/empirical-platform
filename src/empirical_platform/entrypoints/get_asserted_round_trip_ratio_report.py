"""MILESTONE-081 CLI. Read-only: no append path is reachable from here."""
...
_USAGE = (
    "usage: empirical-platform-asserted-round-trip-ratio [--json] "
...
def main() -> None:
    ...
    if as_json:
        print(json.dumps(render_round_trip_ratio_report_json(report), sort_keys=True))
    else:
        print(render_round_trip_ratio_report_text(report), end="")
