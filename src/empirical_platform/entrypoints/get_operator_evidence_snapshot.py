"""MILESTONE-079 CLI. Read-only: no append path is reachable from here."""
...
_USAGE = (
    "usage: empirical-platform-operator-evidence-snapshot [--json] "
...
def main() -> None:
    ...
    if as_json:
        print(json.dumps(render_evidence_snapshot_json(snapshot), sort_keys=True))
    else:
        print(render_evidence_snapshot_text(snapshot), end="")
