"""Real end-to-end Campaign authorization-recording composition root.
...
def run_record_campaign_authorization(
    *,
    ...
    """Record authorization for one Campaign, end-to-end, against real PostgreSQL."""
    with postgres_repository_runtime(config) as runtime:
        handler = RecordCampaignAuthorizationHandler(campaign_repository=runtime.campaigns)
...
def main() -> None:
    """Record authorization for one Campaign from CLI arguments, and print the result."""
    if len(sys.argv) not in (7, 8):
        raise SystemExit(
            "usage: empirical-platform-record-campaign-authorization "
            "<governance_id> <runtime_id> <expected_version> <reason> <actor> <occurred_at_iso> "
            "[correlation_id]"
        )
    result = run_record_campaign_authorization(
