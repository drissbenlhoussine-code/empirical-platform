"""Real end-to-end Campaign resumption composition root.
...
def run_resume_campaign(
    *,
    ...
    """Resume one Campaign, end-to-end, against real PostgreSQL."""
    with postgres_repository_runtime(config) as runtime:
        handler = ResumeCampaignHandler(campaign_repository=runtime.campaigns)
...
def main() -> None:
    """Resume one Campaign from CLI arguments, and print the result."""
    if len(sys.argv) not in (7, 8):
        raise SystemExit(
            "usage: empirical-platform-resume-campaign "
