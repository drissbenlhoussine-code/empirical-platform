"""Real end-to-end EvidencePackage collection-start composition root.

Composes the frozen MILESTONE-038 vertical slice through the shared
...
def run_start_evidence_package_collection(
    *,
    ...
    """Start collection on one EvidencePackage, end-to-end, against real PostgreSQL."""
...
def main() -> None:
    """Start collection on one EvidencePackage from CLI arguments, and print the result."""
    if len(sys.argv) not in (6, 7, 8):
        raise SystemExit(
            "usage: empirical-platform-start-evidence-package-collection "
            "<governance_id> <runtime_id> <expected_version> <actor> <occurred_at_iso> "
            "[correlation_id] [reason]"
        )
    result = run_start_evidence_package_collection(
