"""Real end-to-end Run validation-start composition root.
...
def run_start_run_validation(
    *,
    ...
    """Start validation on one Run, end-to-end, against real PostgreSQL."""
    with postgres_repository_runtime(config) as runtime:
        handler = StartRunValidationHandler(run_repository=runtime.runs)
...
def main() -> None:
    """Start validation on one Run from CLI arguments, and print the result."""
    if len(sys.argv) not in (6, 7, 8):
        raise SystemExit(
            "usage: empirical-platform-start-run-validation "
            "<governance_id> <runtime_id> <expected_version> <actor> <occurred_at_iso> "
            "[correlation_id] [reason]"
        )
    result = run_start_run_validation(
