"""Real end-to-end Campaign suspension composition root.
...
def run_suspend_campaign(
    *,
    ...
    """Suspend one Campaign, end-to-end, against real PostgreSQL."""
    with postgres_repository_runtime(config) as runtime:
        handler = SuspendCampaignHandler(campaign_repository=runtime.campaigns)
...
def main() -> None:
    """Suspend one Campaign from CLI arguments, and print the result."""
    if len(sys.argv) not in (7, 8):
        raise SystemExit(
            "usage: empirical-platform-suspend-campaign "
            "<governance_id> <runtime_id> <expected_version> <reason> <actor> <occurred_at_iso> "
            "[correlation_id]"
        )
    result = run_suspend_campaign(
