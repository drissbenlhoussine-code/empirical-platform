"""Start acquisition for an existing, AUTHORIZED Run: concrete command and handler.
...
class StartRunAcquisitionCommand:
    """Request to transition an existing Run from AUTHORIZED to ACQUIRING."""
...
class StartRunAcquisitionHandler:
    """Loads, starts acquisition on, and persists one Run for one command."""
    ...
    def handle(self, command: StartRunAcquisitionCommand) -> SaveResult:
        """Transition the identified Run to ACQUIRING and persist it."""
        loaded = self._run_repository.get(command.identity)
        run = loaded.aggregate
        run.start_acquisition(
            actor=command.actor,
