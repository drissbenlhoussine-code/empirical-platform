"""MILESTONE-045 contract test: `AddReviewFindingHandler` conforms to the
...
def _assert_conforms_to_command_handler(
    handler: CommandHandler[AddReviewFindingCommand, SaveResult],
) -> None:
...
def test_typed_assignment_proves_protocol_conformance() -> None:
    """Mypy-checked proof: assigning an AddReviewFindingHandler instance to
    a CommandHandler[AddReviewFindingCommand, SaveResult]-typed variable
    only type-checks if the handler structurally satisfies the frozen
    Protocol."""
    handler = AddReviewFindingHandler(
