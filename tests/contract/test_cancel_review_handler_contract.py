"""MILESTONE-049 contract test: `CancelReviewHandler` conforms to the
...
`tests/contract/test_fail_run_handler_contract.py`.
"""
...
def _assert_conforms_to_command_handler(
    handler: CommandHandler[CancelReviewCommand, SaveResult],
) -> None:
...
def test_typed_assignment_proves_protocol_conformance() -> None:
    """Mypy-checked proof: assigning a CancelReviewHandler instance to a
    CommandHandler[CancelReviewCommand, SaveResult]-typed variable only
    type-checks if the handler structurally satisfies the frozen Protocol."""
    handler = CancelReviewHandler(
