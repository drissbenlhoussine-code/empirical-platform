"""MILESTONE-036 contract test: `CreateEvidencePackageHandler` conforms to
...
mirroring the identical pattern `test_create_run_handler_contract.py`
(MILESTONE-033) already established.
"""
...
def test_typed_assignment_proves_protocol_conformance() -> None:
    """Mypy-checked proof: assigning a CreateEvidencePackageHandler instance
    to a CommandHandler[CreateEvidencePackageCommand,
    DomainIdentity[EvidencePackageId]]-typed variable only type-checks if
    CreateEvidencePackageHandler structurally satisfies the frozen Protocol."""
...
def test_handle_method_has_the_frozen_single_parameter_shape() -> None:
    ...
    signature = inspect.signature(CreateEvidencePackageHandler.handle)
    parameters = list(signature.parameters)
    assert parameters == ["self", "command"]


def test_no_inheritance_from_any_command_handler_base_class() -> None:
    """CommandHandler is a Protocol; CreateEvidencePackageHandler must
    ...
    assert CreateEvidencePackageHandler.__bases__ == (object,)
