"""MILESTONE-032 contract test: `PrepareCampaignForAuthorizationHandler` conforms to
...
already established in `tests/contract/test_create_campaign_handler_contract.py`
and `tests/contract/test_get_campaign_handler_contract.py`.
"""
...
def _assert_conforms_to_command_handler(
    handler: CommandHandler[PrepareCampaignForAuthorizationCommand, SaveResult],
) -> None:
...
def test_typed_assignment_proves_protocol_conformance() -> None:
    """Mypy-checked proof: assigning a PrepareCampaignForAuthorizationHandler
    instance to a CommandHandler[PrepareCampaignForAuthorizationCommand,
    SaveResult]-typed variable only type-checks if the handler structurally
    satisfies the frozen Protocol."""
    handler = PrepareCampaignForAuthorizationHandler(
...
def test_handle_method_has_the_frozen_single_parameter_shape() -> None:
    ...
    signature = inspect.signature(PrepareCampaignForAuthorizationHandler.handle)
    parameters = list(signature.parameters)
    assert parameters == ["self", "command"]


def test_no_inheritance_from_any_command_handler_base_class() -> None:
    """CommandHandler is a Protocol; PrepareCampaignForAuthorizationHandler must
    satisfy it structurally, not through inheritance -- matching M027's own
    frozen design intent (no base class, no marker interface)."""
    assert PrepareCampaignForAuthorizationHandler.__bases__ == (object,)
