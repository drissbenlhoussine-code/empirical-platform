"""MILESTONE-040 contract test:
`RecordEvidencePackageArtifactReferenceHandler` conforms to the frozen
...
`tests/contract/test_record_evidence_package_criterion_result_handler_contract.py`.
"""
...
def _assert_conforms_to_command_handler(
    handler: CommandHandler[RecordEvidencePackageArtifactReferenceCommand, SaveResult],
) -> None:
...
def test_typed_assignment_proves_protocol_conformance() -> None:
    """Mypy-checked proof: assigning a
    RecordEvidencePackageArtifactReferenceHandler instance to a
...
def test_handle_method_has_the_frozen_single_parameter_shape() -> None:
    ...
    signature = inspect.signature(RecordEvidencePackageArtifactReferenceHandler.handle)
    parameters = list(signature.parameters)
    assert parameters == ["self", "command"]


def test_no_inheritance_from_any_command_handler_base_class() -> None:
    """CommandHandler is a Protocol; RecordEvidencePackageArtifactReferenceHandler
    must satisfy it structurally, not through inheritance -- matching M027's
    own frozen design intent (no base class, no marker interface)."""
    assert RecordEvidencePackageArtifactReferenceHandler.__bases__ == (object,)
