"""MILESTONE-039 contract test:
`RecordEvidencePackageCriterionResultHandler` conforms to the frozen
...
def _assert_conforms_to_command_handler(
    handler: CommandHandler[RecordEvidencePackageCriterionResultCommand, SaveResult],
) -> None:
...
def test_typed_assignment_proves_protocol_conformance() -> None:
    ...
    RecordEvidencePackageCriterionResultHandler instance to a
    CommandHandler[RecordEvidencePackageCriterionResultCommand,
...
def test_handle_method_has_the_frozen_single_parameter_shape() -> None:
    ...
    signature = inspect.signature(RecordEvidencePackageCriterionResultHandler.handle)
    parameters = list(signature.parameters)
    assert parameters == ["self", "command"]


def test_no_inheritance_from_any_command_handler_base_class() -> None:
    """CommandHandler is a Protocol; RecordEvidencePackageCriterionResultHandler
    ...
    assert RecordEvidencePackageCriterionResultHandler.__bases__ == (object,)
