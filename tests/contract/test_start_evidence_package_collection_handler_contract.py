"""MILESTONE-038 contract test: `StartEvidencePackageCollectionHandler`
conforms to the frozen MILESTONE-027 `CommandHandler` Protocol.
...
already established in `tests/contract/test_authorize_run_handler_contract.py`.
"""
...
def _assert_conforms_to_command_handler(
    handler: CommandHandler[StartEvidencePackageCollectionCommand, SaveResult],
) -> None:
...
def test_typed_assignment_proves_protocol_conformance() -> None:
    """Mypy-checked proof: assigning a StartEvidencePackageCollectionHandler
    instance to a
    CommandHandler[StartEvidencePackageCollectionCommand, SaveResult]-typed
    variable only type-checks if the handler structurally satisfies the
    frozen Protocol."""
...
def test_handle_method_has_the_frozen_single_parameter_shape() -> None:
    ...
    signature = inspect.signature(StartEvidencePackageCollectionHandler.handle)
    parameters = list(signature.parameters)
    assert parameters == ["self", "command"]


def test_no_inheritance_from_any_command_handler_base_class() -> None:
    """CommandHandler is a Protocol; StartEvidencePackageCollectionHandler
    ...
    assert StartEvidencePackageCollectionHandler.__bases__ == (object,)
