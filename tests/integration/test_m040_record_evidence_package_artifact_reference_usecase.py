"""MILESTONE-040 real-PostgreSQL integration tests for
...
`RecordEvidencePackageArtifactReferenceHandler` -> `EvidencePackage`
...
pattern `tests/integration/test_m039_record_evidence_package_criterion_result_usecase.py`
already established.

The deterministic optimistic-concurrency conflict scenario is frozen
exactly by the M040 design (Section 6) -- the reverse pairing of M039's
own mechanism: `add_artifact_reference()` operates on COLLECTING, which
has a genuine, now-frozen sibling method -- `add_criterion_result()` --
available as the interfering write. The interfering write independently
loads the same identity and calls `EvidencePackage.add_criterion_result()`
-- never `add_artifact_reference()` itself -- to advance the persisted
version while preserving COLLECTING, so the command under test can still
execute its own `add_artifact_reference()` call before the stale-version
`save()` fails. `add_criterion_result()` is test setup only here; it is
never invoked by any production code in this milestone.
...
def _config() -> PostgreSQLConfigSnapshot:
    return PostgreSQLConfigSnapshot(
        ...
        application_name="empirical-platform-m040-test",
    )
    ...
    *,
    ...
    CommandEntryPoint(create_campaign_handler)(
        CreateCampaignCommand(
            ...
            scope_statement="seed campaign for M040 tests",
        )
    )
...
def test_golden_path_records_artifact_reference_via_command_entry_point_and_real_repository(
    campaign_repo: PostgresCampaignRepository,
    run_repo: PostgresRunRepository,
    evidence_package_repo: PostgresEvidencePackageRepository,
) -> None:
    ...
    recorded = loaded.aggregate.artifact_references[0]
    assert recorded.value == "s3://bucket/artifact-1"
...
def test_invalid_state_raises_domain_value_error(
    ...
    rejects add_artifact_reference() before save() is ever reached."""
    create_campaign_handler = CreateCampaignHandler(
        campaign_repository=campaign_repo,
...
def test_duplicate_value_raises_domain_value_error(
    campaign_repo: PostgresCampaignRepository,
    run_repo: PostgresRunRepository,
    evidence_package_repo: PostgresEvidencePackageRepository,
) -> None:
...
def test_stale_expected_version_raises_genuine_optimistic_concurrency_conflict(
    ...
    # and advance the persisted version via add_criterion_result() -- the
    # reverse pairing of M039's own mechanism, the only existing
    # EvidencePackage mutation, other than add_artifact_reference itself,
    # that bumps AggregateVersion without changing lifecycle state,
    # preserving COLLECTING so the command under test can still execute
    # add_artifact_reference() afterward. Test setup only; never invoked by
    # any production code.
    interfering = evidence_package_repo.get(identity)
    interfering.aggregate.add_criterion_result(
        CriterionResult(
            evidence_package_id=identity.governance_id,
            criterion_id="interfering-writer",
    ...
    # The command-under-test's own artifact reference was never persisted --
    # the interfering writer's criterion result remains the only durable
