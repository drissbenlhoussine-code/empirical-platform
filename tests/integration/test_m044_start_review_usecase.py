"""MILESTONE-044 real-PostgreSQL integration tests for `StartReviewHandler`.
...
`StartReviewHandler` -> `Review` aggregate -> the real, frozen
...
`tests/integration/test_m043_get_review_usecase.py` already established.

The "conflict" scenario is frozen exactly by the M044 design (Section 6):
`Review` has no method analogous to a state-preserving mutation available
while `ASSIGNED` -- `start()` is the only method that operates on that
state, and it changes state away from `ASSIGNED`. Two independently loaded
callers racing the same transition therefore produce a domain-level
`ValueError` (the second caller's own `start()` call is invalid against
the now-`IN_PROGRESS` durable state, so it never reaches `save()`), not a
repository-level `OptimisticConcurrencyConflict` -- this is an
independently-derived, disclosed boundary of this specific transition, not
a gap, mirroring the identical, already-accepted M038 resolution.
`OptimisticConcurrencyConflict` propagation itself remains proven at the
unit level (`tests/unit/test_start_review_usecase.py`) via a fake
repository unconstrained by the aggregate's own state machine.
...
def _config() -> PostgreSQLConfigSnapshot:
    return PostgreSQLConfigSnapshot(
        ...
        application_name="empirical-platform-m044-test",
    )
    ...
    *,
    ...
    CommandEntryPoint(
        ...
        CreateCampaignCommand(
            ...
            scope_statement="seed campaign for M044 tests",
        )
    )
...
def test_golden_path_starts_review_via_command_entry_point_and_real_repository(
    campaign_repo: PostgresCampaignRepository,
    run_repo: PostgresRunRepository,
    evidence_package_repo: PostgresEvidencePackageRepository,
    review_repo: PostgresReviewRepository,
) -> None:
    identity = _persist_review(
        campaign_repo,
        run_repo,
    ...
    handler = StartReviewHandler(review_repository=review_repo)
...
def test_two_racing_callers_second_start_raises_domain_value_error(
    campaign_repo: PostgresCampaignRepository,
    run_repo: PostgresRunRepository,
    evidence_package_repo: PostgresEvidencePackageRepository,
    review_repo: PostgresReviewRepository,
    ...
    the second independently-loaded caller's own start() call is invalid
    once the first caller has already advanced the durable state to
    IN_PROGRESS, so it raises a domain ValueError before save() is ever
    ...
    first_handler = StartReviewHandler(review_repository=review_repo)
    ...
    second_handler = StartReviewHandler(review_repository=review_repo)
