"""MILESTONE-040 behavioral tests for
`RecordEvidencePackageArtifactReferenceCommand`/`RecordEvidencePackageArtifactReferenceHandler`.
...
`ArtifactReference(value=command.value)` -- unlike M039's `CriterionResult`,
there is no ownership-derivation question here, since `ArtifactReference`
carries no `evidence_package_id` field at all (design Section 4) -- invoke
`EvidencePackage.add_artifact_reference()` exactly once, then call
`EvidencePackageRepository.save()` exactly once with the mutated aggregate
...
for every collaborator (frozen M029 invariant), including propagation of
`OptimisticConcurrencyConflict` via a fake repository, and that the
...
def _command(
    ...
    *,
    ...
    value: str = "s3://bucket/artifact-1",
) -> RecordEvidencePackageArtifactReferenceCommand:
    return RecordEvidencePackageArtifactReferenceCommand(
        identity=identity,
...
class _RecordingEvidencePackageRepository:
    ...
    def add(self, aggregate: EvidencePackage) -> SaveResult:
        raise AssertionError(
            "add() must not be called by RecordEvidencePackageArtifactReferenceHandler"
        )
...
def _handler(repository: object) -> RecordEvidencePackageArtifactReferenceHandler:
    return RecordEvidencePackageArtifactReferenceHandler(evidence_package_repository=repository)  # type: ignore[arg-type]


# --- A. Command contract tests ---


def test_command_preserves_all_three_fields_unchanged() -> None:
    identity = _identity()
    version = AggregateVersion(3)
    command = RecordEvidencePackageArtifactReferenceCommand(
        identity=identity,
        expected_persisted_version=version,
        value="s3://bucket/ref-99",
    )

    assert command.identity is identity
    assert command.expected_persisted_version is version
    assert command.value == "s3://bucket/ref-99"


def test_command_contains_no_additional_fields() -> None:
    assert set(RecordEvidencePackageArtifactReferenceCommand.__slots__) == {  # type: ignore[attr-defined]
        "identity",
...
def test_command_has_no_evidence_package_id_field() -> None:
    """Critical: unlike M039's CriterionResult-backed command, ArtifactReference
    carries no evidence_package_id -- there is no ownership-derivation
    question, and no field for it exists on the command at all."""
    assert "evidence_package_id" not in RecordEvidencePackageArtifactReferenceCommand.__slots__  # type: ignore[attr-defined]


def test_command_is_immutable() -> None:
    command = _command(_identity())
    with pytest.raises(AttributeError):
        command.value = "someone-else"  # type: ignore[misc]


def test_command_construction_performs_no_business_validation() -> None:
    """No duplicated domain validation; raw empty-string value is accepted at
    construction -- ArtifactReference itself validates later."""
    command = RecordEvidencePackageArtifactReferenceCommand(
        identity=_identity(),
...
def test_typed_conformance_check() -> None:
    """Mypy-checked proof that RecordEvidencePackageArtifactReferenceHandler
    conforms to
    CommandHandler[RecordEvidencePackageArtifactReferenceCommand,
    SaveResult] without inheritance (structural typing only)."""
    identity = _identity()
    ...
    handler: RecordEvidencePackageArtifactReferenceHandler = _handler(repository)
...
def test_add_artifact_reference_called_with_exact_command_value() -> None:
    identity = _identity()
    ...
    repository = _RecordingEvidencePackageRepository(loaded, save_result)
    handler = _handler(repository)
    command = _command(identity, value="s3://bucket/artifact-xyz")

    handler.handle(command)

    assert len(package.artifact_references) == 1
    recorded = package.artifact_references[0]
    assert recorded.value == "s3://bucket/artifact-xyz"
...
def test_no_criterion_result_seal_or_start_collection_call_occurs() -> None:
    """The mutated aggregate carries only the newly recorded artifact
    reference -- no criterion result, no seal, no re-invocation of
    start_collection."""
    identity = _identity()
    package = _collecting_package(identity)
    loaded = LoadedAggregate(aggregate=package, persisted_version=AggregateVersion(1))
...
def test_duplicate_value_propagates_and_save_never_called() -> None:
    identity = _identity()
    package = _collecting_package(identity)
    package.add_artifact_reference(ArtifactReference(value="s3://bucket/dup"))
    ...
    with pytest.raises(ValueError, match="already exists"):
        handler.handle(
            _command(
                identity, value="s3://bucket/dup", expected_persisted_version=AggregateVersion(2)
            )
        )
